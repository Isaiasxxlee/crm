...
import { planScenarios } from "./scenarios.mjs";

const projectDir = fileURLToPath(new URL("../..", import.meta.url));
const PORT = 40000 + Math.floor(Math.random() * 4000);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const RUN_ID = `plans${Date.now()}${Math.floor(Math.random() * 100000)}`;
...
test("plans and subscriptions integration", async (t) => {
  const pool = new pg.Pool({ connectionString: process.env.DATABASE_URL });
  try {
    await planScenarios(t, api, agentA, agentB, pool);
  } finally {
    await pool.end();
  }
});
